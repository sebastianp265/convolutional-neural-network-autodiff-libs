...
@testset "KM3" begin
    X_train = load("../data/KM3/imdb_dataset_prepared.jld2", "X_train")[:, 1:2560]
    y_train = load("../data/KM3/imdb_dataset_prepared.jld2", "y_train")[:, 1:2560]
    ...
    test = load("../data/KM3/imdb_dataset_prepared.jld2", "X_train")
    ...
    flux_model = Flux.Chain(
        ...
        Flux.MaxPool((8,)),
        Flux.flatten,
        Flux.Dense(128, 1, Flux.sigmoid, init=fixed_glorot)
    )
    my_model = Chain(
